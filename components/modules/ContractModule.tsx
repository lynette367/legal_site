...
import { moduleExamples } from "@/data/exampleData";
...
export function ContractModule() {
  const example = moduleExamples.contract;
  const { callAIApi, isProcessing } = useAIModule();
  const [contractType, setContractType] = useState(contractTypes[3].value);
  const [requirements, setRequirements] = useState("");
  const [verified, setVerified] = useState(false);
  const [panelMessage, setPanelMessage] = useState("");
  const [draft, setDraft] = useState<string | null>(example.answer);

  const handleUseExample = () => {
    setContractType(contractTypes[3].value);
      ...
      <section className="rounded-3xl border border-border-lavender bg-bg-card p-8 shadow-soft">
        ...
        <div className="mt-6 space-y-4">
          ...
          <textarea
            ...
            placeholder={`Example: ${example.prompt}`}
            value={requirements}
          ...
          <div className="rounded-2xl border border-border-lavender/70 bg-white/90 p-5">
            ...
            <div className="prose prose-sm max-w-none">
              <div className="whitespace-pre-wrap text-text-primary bg-white rounded-xl p-4 border border-border-lavender/50 text-sm leading-relaxed max-h-[500px] overflow-y-auto">
                {draft ? (
                  draft
                ) : (
                  <p className="whitespace-pre-line text-text-primary/70">
                    {panelMessage || "Your answer will appear here."}
                  </p>
                )}
              </div>
            </div>
